...
function GaugeChartComponent(_ref) {
  ...
  var ref = (0, _d3hook.useD3)(function (svg) {
    svg.selectAll('rect').data(builtDataObjectWithX).enter().append('rect').attr('width', function (d) {
      return d.width;
    }).attr('x', function (d) {
      return d.x;
    ...
    if (showBarValues) {
      svg.selectAll('text').data(builtDataObjectWithX).enter().append("text").text(function (d) {
        return d.text;
